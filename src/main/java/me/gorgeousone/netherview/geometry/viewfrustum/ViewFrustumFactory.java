...
public final class ViewFrustumFactory {
	...
	private ViewFrustum createFrustum2(Vector viewPoint, AxisAlignedRect portalRect, int frustumLength) {
		
		boolean isPlayerAbovePortal = isPlayerBehindPortal(viewPoint, portalRect);
		
		Vector portalNormal = portalRect.getPlane().getNormal();
		Vector playerFacingToPortal = portalNormal.multiply(isPlayerAbovePortal ? 1 : -1);
	...
	public static boolean isPlayerBehindPortal(Vector viewPoint, AxisAlignedRect portalRect) {
		...
		switch (portalRect.getAxis()) {
			
			case X:
				return viewPoint.getZ() < portalPos.getZ();
			case Z:
				return viewPoint.getX() < portalPos.getX();
			default:
				return viewPoint.getY() < portalPos.getY();
		}
	}
